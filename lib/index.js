      ...
      return getDbMetadata(dialect, tableName, schema, db)
        .then(function(metadata) {
          return (tableExists(metadata) ?
            checkTableStructure(dialect, tableName, schema, metadata, db) :
  ...
  _.forEach(schema.properties, function(property, name) {
    ...
    if ($ref) {
      ...
      if (metadata.tablesWithPrimaryKey[referenceTableName] && !(metadata.tablesWithForeignKeys[tableName] &&
        _.find(metadata.tablesWithForeignKeys[tableName], 'referenceColumn', foreignKey))) {
  ...
  _.forEach(schema.properties, function(property, name) {
    ...
    if (!metadata.columns[fieldName]) {
      commands.push('ALTER TABLE [' + tableName + '] ADD ['
        + fieldName + ']' + fieldType);
    } else if (canAlterColumn(metadata.columns[fieldName], property)) {
